...
final class DatabaseProgressStore implements ProgressStoreInterface
{
    ...
    public function setProcessed(string $jobId, int $processed, ?int $percent = null): void
    {
        ...
        $resolvedPercent   = $percent !== null && $percent >= 0 ? $percent : 0;
    ...
    public function requestCancellation(string $jobId): bool
    {
        ...
        return $this->withReconnectRetry(function () use ($resolvedJobId): bool {
            ...
            $now    = $this->now();
            $insert = $connection->query()->insert($this->schema->table, [
                $this->schema->jobIdColumn                   => $resolvedJobId,
                $this->schema->statusColumn                  => QueueProgressStatus::QUEUED,
                $this->schema->totalColumn                   => null,
                $this->schema->processedColumn               => 0,
                $this->schema->percentColumn                 => 0,
                $this->schema->stepPercentColumn             => 1,
                $this->schema->attemptColumn                 => 1,
                $this->schema->cancelRequestedDatetimeColumn => $now,
                $this->schema->startedDatetimeColumn         => null,
                $this->schema->createdDatetimeColumn         => $now,
                $this->schema->updatedDatetimeColumn         => $now,
                $this->schema->finishedDatetimeColumn        => null,
            ])->compile();

            $quoter = $connection->driver()->createQuoter();
            $update = sprintf(
                '%s = :cancel_update, %s = :time_update',
                $quoter->ident($this->schema->cancelRequestedDatetimeColumn),
                $quoter->ident($this->schema->updatedDatetimeColumn),
            );
            // One atomic statement: unchanged rows and concurrent cancellation requests
            // must not be mistaken for a missing job. Preserve all other progress fields.
            $conflict = match ($connection->driver()->name()) {
                'mysql', 'mariadb'            => ' ON DUPLICATE KEY UPDATE ' . $update,
                'sqlite', 'postgres', 'pgsql' => sprintf(
                    ' ON CONFLICT (%s) DO UPDATE SET %s',
                    $quoter->ident($this->schema->jobIdColumn),
                    $update,
                ),
                default => throw new QueueException('Unsupported database driver for queue cancellation.'),
            };
            $connection->execute($insert->sql . $conflict, [
                ...$insert->bindings,
                'cancel_update' => $now,
                'time_update'   => $now,
            ]);
