...
- композиция application-level handlers через `CompositeQueueJobHandler`
...
## Выбор хранилища прогресса

Очередь хранит задания, а progress-store — их состояние и запросы отмены.
Это независимые настройки: memory-очередь не отключает прогресс автоматически.
...
// Общая БД для HTTP-процесса и worker. $connections — ConnectionManagerInterface.
$database = $factory->create([
    'progress_driver' => 'database',
    'progress_connection' => 'default',
], $connections);

// Только внутри текущего процесса и экземпляра объекта.
$memory = $factory->create(['progress_driver' => 'memory']);

// Явное отключение наблюдения за прогрессом и запросов отмены.
$disabled = $factory->create(['progress_driver' => 'null']);
```

Если `progress_driver` отсутствует, выбирается `database`, даже если `driver=memory`.
PHP `null`, пустая строка и неизвестные значения приводят к ошибке, не к fallback.
Для memory один и тот же экземпляр передают worker и всем читателям/отправителям
отмены; фабрика не кеширует объекты. Для раздельных процессов используйте database.
...
- [Composite Handler](docs/09-composite-handler.md)
- [Обновление progress-store](docs/10-progress-stores-upgrade.md)
